...
var servidorNode = 'localhost:3000';
var servidorRuby = '23.96.5.154:3000';

var token ="";

//var urlLojas = 'http://rdlabs-integ:8280/services/PLANTA_LOJA/getStore';
//var urlLojas = 'http://rdlabs-integ:8280/getStores'
var urlLojas = 'http://'+servidorNode+'/api/ground_list/';
var urlLayers = 'http://'+servidorNode+'/api/layer_list/';
var urlLayerConfg = 'http://'+servidorNode+'/api/plan_layers';
var urlObjectConfg = 'http://'+servidorNode+'/api/plan_objects';
var urlgetObjTemplates = 'http://'+servidorNode+'/template_list/';
...
var path = 'http://'+servidorNode+'/api/pathLine/22.33/-44.32';
        ...
        if(validaCampo("wallpaper_posBase",confg)){
          ...
          viewer.entities.add({
            ...
            wall : {
                ...
                positions : Cesium.Cartesian3.fromDegreesArrayHeights([coord[w_x][0]+plusX, coord[w_x][1]+plusY, confg.wallpaper_altura,
                                                                       coord[w_x2][0]+plusX, coord[w_x2][1]+plusY, confg.wallpaper_altura]),
                material : objMatrial
            }
          });
...
function desenhaLayerLine(strUrl, objLayer, objPai, foco){
   var confgLayer = JSON.parse(objLayer.custom_confg);
    ...
    success: function( data ) {
      $.each( data.features, function( key, val ) {
        ...
        trackHome = viewer.entities.add({
          ...
          //description : "Sem Conteudo",
          polyline : {
              positions : new Cesium.Cartesian3.fromDegreesArray([val.geometry.coordinates[0][0],
                                                                  val.geometry.coordinates[0][1],
                                                                  val.geometry.coordinates[1][0],
                                                                  val.geometry.coordinates[1][1]]),
              //outlineColor : Cesium.Color.fromCssColorString(cor_borda),
              //outlineColor : Cesium.Color.fromCssColorString(cor),
              //outlineWidth :1,
              extrudedHeight : nuExtruded + nuAltura,
...
function desenhaLayerPath(strUrl){
    ...
    success: function( data ) {
      $.each( data.features, function( key, val ) {
        ...
        trackHome = viewer.entities.add({
          ...
          //description : "Sem Conteudo",
          polyline : {
              positions : new Cesium.Cartesian3.fromDegreesArray(coord),

              material : Cesium.Color.fromAlpha(Cesium.Color.fromCssColorString("#FF0000"), 1) 
          },
        });

        entidades.getById("Path" + idObj).show = true; 
...
function urlPolygon(piso, loja, layer){
  //return 'http://'+ servidor +'/geoserver/brMalls/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=brMalls:vw_polygon&outputFormat=application%2Fjson&viewparams=floor:'+piso+';shopping:'+loja +';layer:'+ layer; //Geoserver URL  
  return 'http://'+servidorNode+'/api/polygon/'+piso+'/'+loja+'/'+layer;
}

function urlLinestring(piso, loja, layer){

  //return 'http://'+ servidor +'/geoserver/brMalls/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=brMalls:vw_linestring&outputFormat=application%2Fjson&viewparams=floor:'+piso+';shopping:'+loja +';layer:'+ layer; //Geoserver URL  
  return 'http://'+servidorNode+'/api/linestring/'+piso+'/'+loja+'/'+layer;
}
...
//--------------------------------------------------------------------------------------Design Objeto--------------
  contador = 0 
    ...
    str = str + "           </table>";
    str = str + "            <br> <input type='button' onclick=\"setCustomConfg('fieldsObjs', "+ obj.properties.id +")\" value='Salvar'>";
    str = str + "       </form>";
    str = str + "     </span></li>";
    ...
    success: function( data ) {
        ...
        $.each( data, function( key, val ) {
          ...
          default:
            ...
            if (val.title =='ROTA'){
              desenhaLayerLine(urlLayer, val, layer, focus);
            }else{
